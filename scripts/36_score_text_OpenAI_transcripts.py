#!/usr/bin/env python3
# scripts/36_score_text_OpenAI_transcripts.py
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Score transcript text units with OpenAI.")
    ...
    for t in tickers:
        ...
        units_path = out_dir / "text_oa_transcripts_units.csv"
        ...
        ev_path = out_dir / "text_oa_transcripts_event_long.csv"
        _maybe_write(ev_long, ev_path, overwrite=bool(args.overwrite))
        ev_wide = pivot_wide(ev_long, suffix_prefix="tr_oa_")
        ev_wide_path = out_dir / "text_oa_transcripts_event_wide.csv"
        ...
        (out_dir / "text_oa_transcripts.meta.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")

        print(f"[OK] {t}: OpenAI transcript units -> {units_path} rows={len(out_units):,}")
        print(f"[OK] {t}: OpenAI transcript event -> {ev_wide_path} events={meta['events_scored']}")
