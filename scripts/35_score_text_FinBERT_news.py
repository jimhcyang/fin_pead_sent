#!/usr/bin/env python3
# scripts/35_score_text_FinBERT_news.py
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Score news text units with FinBERT.")
    ...
    for t in tickers:
        ...
        units_path = out_dir / "text_finbert_news_units.csv"
        ...
        ev_path = out_dir / "text_finbert_news_event_long.csv"
        _maybe_write(ev_long, ev_path, overwrite=bool(args.overwrite))
        ev_wide = pivot_wide(ev_long, suffix_prefix="news_finbert_")
        ev_wide_path = out_dir / "text_finbert_news_event_wide.csv"
        ...
        (out_dir / "text_finbert_news.meta.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")

        print(f"[OK] {t}: FinBERT news units -> {units_path} rows={len(out_units):,}")
        print(f"[OK] {t}: FinBERT news event -> {ev_wide_path} events={meta['events_scored']}")
